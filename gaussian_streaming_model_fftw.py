...
class GaussianStreamingModel(VelocityMoments):
    ...
    def setup_config_vels(self):
        ...
        _integrand = loginterp(self.vktable[:,0], self.vktable[:,1]+self.vktable[:,2]+self.vktable[:,3])(self.kint)
        ...
        _integrand = loginterp(self.sparktable[10:,0], self.s0[10:,1]+self.s0[10:,2]+self.s0[10:,3])(self.kint)
        qint, s0matter = self.sph_gsm.sph(0,_integrand)
        self.s0matter = np.interp(self.rint, qint, s0matter)

        _integrand = loginterp(self.sparktable[5:,0], self.s2[5:,1]+self.s2[5:,2]+self.s2[5:,3])(self.kint)
    ...
    def compute_cumulants(self, b1, b2, bs, alpha, alpha_v, s2fog, alpha_s0, alpha_s2):
        '''
        ...
        self.veft = self.vmatter + b1*self.vtable[:,3] + b1**2*self.vtable[:,4]\
        + b2*self.vtable[:,5] + b1*b2*self.vtable[:,6] \
        + bs*self.vtable[:,8] + b1*bs*self.vtable[:,9]\
        + alpha_v*self.vct
        
        self.s0eft =  self.s0matter + b1*self.s0table[:,3] + b1**2*self.s0table[:,4]\
                                                           + b2*self.s0table[:,5] \
                                                           + bs*self.s0table[:,8] \
                                                           + alpha_s0 *self.xitable[:,0] \
                                                           + s2fog
        self.s0eft += (self.Xddot + self.Xloopddot + 2*b1*self.X10ddot + 2*bs*self.Xs2ddot)[-1] #add in 0-lag term
                                                           
        self.s2eft =  self.s2matter + b1*self.s2table[:,3] + b1**2*self.s2table[:,4]\
                                                           + b2*self.s2table[:,5] \
                                                           + bs*self.s2table[:,8] \
                                                           + alpha_s2 * self.s2ct


    def compute_xi_rsd(self, sperp, spar, f, b1, b2, bs, alpha, alpha_v, s2fog, alpha_s0, alpha_s2, rwidth=100, Nint=10000, update_cumulants=True):

        # If cumulants have already been computed, skip this step:
        if update_cumulants:
            self.compute_cumulants(b1, b2, bs, alpha, alpha_v, s2fog, alpha_s0, alpha_s2)
    ...
    def compute_xi_ell(self, s, f, b1, b2, bs, alpha, alpha_v, s2fog, alpha_s0, alpha_s2, rwidth=100, Nint=10000, ngauss=4):
        '''
        ...
        for ii, nu in enumerate(nus_calc):
            xi_nu = self.compute_xi_rsd(s*np.sqrt(1-nu**2),s*nu, f, b1, b2, bs, alpha, alpha_v, s2fog, alpha_s0, alpha_s2, rwidth=rwidth, Nint=Nint, update_cumulants=False)
