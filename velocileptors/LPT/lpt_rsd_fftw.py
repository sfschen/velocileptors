...
class LPT_RSD:
    ...
    '''
        Optimized variant of velocileptors.LPT.lpt_rsd_fftw.LPT_RSD.

        Functionally equivalent to the original, but p_integrals / p_integral_fixedbias
        replace 45 per-call _G0_l-style helper invocations with 9 band-matrix matmuls
        against a single precomputed k^(2j) * powerfacs table, and swap interp1d for
        a manual linear blend. setup_rsd_facs also vectorizes the hyp2f1/gamma table.
        
        Currently only supports a limited set of features from the original velocileptors.LPT.lpt_rsd_fftw (though, the most relevant ones).
    ...
    def setup_rsd_facs(self,f,nu,D=1,nmax=10):
        ...
        self._nmax_setup = nmax

        # hyp2f1 / gamma tables (vectorized over (n,m))
        n_arr = np.arange(self.jn+nmax)[:, None]
        m_arr = np.arange(self.jn+nmax)[None, :]
        self.hyp1 = hyp2f1(0.5-n_arr, -n_arr, 0.5-m_arr-n_arr, self.ic2)
        self.hyp2 = hyp2f1(1.5-n_arr, -n_arr, 0.5-m_arr-n_arr, self.ic2)
        self.fnms = gamma(m_arr+n_arr+0.5)/gamma(m_arr+1)/gamma(n_arr+0.5)/gamma(1-m_arr+n_arr)
        ...
        # Band matrices: band[l, j] = coeffs[l, j-l] for 0 <= j-l < nmax, else 0.
        # Shape (jn, jn+nmax). Then  G_arr[l, :] = (band @ kpow_pf)[l, :] with
        # kpow_pf[j, :] = k^(2j) * powerfacs[j, :].
        nrows = self.jn
        ncols = self.jn + nmax
        idx_l = np.arange(nrows)[:, None]
        idx_n = np.arange(nmax)[None, :]
        def _build_band(coeffs):
            band = np.zeros((nrows, ncols))
            band[idx_l, idx_l + idx_n] = coeffs
            return band
        self._G0_band       = _build_band(self.G0_l_ns)
        self._dG0dA_band    = _build_band(self.dG0dA_l_ns)
        self._d2G0dA2_band  = _build_band(self.d2G0dA2_l_ns)
        self._dG0dC_band    = _build_band(self.dG0dC_l_ns)
        self._d2G0dCdA_band = _build_band(self.d2G0dCdA_l_ns)
        self._d2G0dC2_band  = _build_band(self.d2G0dC2_l_ns)
        self._d3G0dA3_band  = _build_band(self.d3G0dA3_l_ns)
        self._d3G0dCdA2_band= _build_band(self.d3G0dCdA2_l_ns)
        self._d4G0dA4_band  = _build_band(self.d4G0dA4_l_ns)
    ...
    def _compute_G_arrays(self, k, nmax):
        '''
        Vectorized replacement for the 9 list comprehensions
            [self._G0_l(ii,k,nmax=nmax) for ii in range(self.jn)]
        etc. in p_integrals. Returns 9 (jn, N) ndarrays.
        '''
        jn = self.jn
        kq = k*self.qint
        # kpow_pf[j, :] = k^(2j) * powerfacs[j, :]
        kpow = k**(2*np.arange(jn+nmax))
        kpow_pf = kpow[:, None] * self.powerfacs

        # One matmul per G-function. band @ kpow_pf -> (jn, N).
        G0       = self._G0_band       @ kpow_pf
        dGdA     = (self._dG0dA_band    @ kpow_pf) / kq
        dGdC     = (self._dG0dC_band    @ kpow_pf) / kq
        d2GdA2   = (self._d2G0dA2_band  @ kpow_pf) / kq**2
        d2GdCdA  = (self._d2G0dCdA_band @ kpow_pf) / kq**2
        d2GdC2   = (self._d2G0dC2_band  @ kpow_pf) / kq**2
        d3GdA3   = (self._d3G0dA3_band  @ kpow_pf) / kq**3
        d3GdCdA2 = (self._d3G0dCdA2_band@ kpow_pf) / kq**3
        d4GdA4   = (self._d4G0dA4_band  @ kpow_pf) / kq**4
        return G0, dGdA, dGdC, d2GdA2, d2GdCdA, d2GdC2, d3GdA3, d3GdCdA2, d4GdA4

    @staticmethod
    def _shift_down(mat, m):
        '''Return out such that out[i, :] = mat[i-m, :] for i >= m, else 0.
        Same semantics as the zero-padded list trick "list(mat) + [0]*m"
        with negative Python indexing producing zeros when ii < m.'''
    ...
    def p_integrals(self, k, nmax=8, old=False):
        ...
        # Shifted views: X_m[i, :] = X[i-m, :] if i >= m else 0, shape (jn, N).
        # Replaces the "list + [0]*m + negative indexing" pattern from the
        # original code with explicit zero-padding compatible with 2D arrays.
        ...
        # All (jn, N) arrays via broadcasting. The for-loop below indexes
        # G01s[l] etc., which on a 2D ndarray returns a 1D view of shape (N,)
        # — same semantics as the previous list-of-arrays.
        ...
        # Vectorized over l: all of mu0..mu4, nq1, mu_nq1, nq2, mu2_nq1 are (jn, N).
        Anu = self.Anu; Bnu = self.Bnu
        mu0     = G0s
        mu1     = G01s
        mu2     = G02s
        mu3     = G03s
        ...
        nq2     = Anu**2 * G02s + 2*Anu*Bnu * G11s + Bnu**(2+2*old) * G20s
        mu2_nq1 = Anu * G03s + Bnu * G12s

        # Build bias_integrands_all with shape (jn, num_power_components, N) in one
        # vectorized pass. Per-l tensors (mu0, Xlin_gt, etc.) broadcast against the
        # (jn, N) tensors above and the (N,) Lagrangian kernels.
        Xg = self.Xlin_gt; Yg = self.Ylin_gt
        bias_integrands_all = np.zeros((self.jn, self.num_power_components, self.N))

        # Summation order matches the original's three += steps (za, Aloop, Wloop)
        # to keep floating-point output bit-close.
        ...
        # IR exponent * (-2/(k q))^l, broadcast over component axis.
        kq_fac = -2.0 / (k * self.qint)                          # (N,)
        factor_l = expon[None, :] * (kq_fac[None, :] ** np.arange(self.jn)[:, None])  # (jn, N)
        bias_integrands_all *= factor_l[:, None, :]
        # l=0: subtract value at q=qint[-1] (large-q DC removal).
        bias_integrands_all[0] -= bias_integrands_all[0, :, -1:]

        # Still one FFTLog per l (different Bessel order).
        for l in range(self.jn):
            ktemps, bias_ffts = self.sph.sph(l, bias_integrands_all[l])
            ...
            w = (k - x0) / (x1 - x0)
            ret += bias_ffts[:, idx-1] + w * (bias_ffts[:, idx] - bias_ffts[:, idx-1])
    ...
    def make_pltable(self,f, apar = 1, aperp = 1, ngauss = 3, kv = None, kmin = 1e-2, kmax = 0.25, nk = 50, nmax=5):
        '''
    ...
    def p_integral_fixedbias(self, k, bvec, nmax=5):
        '''
